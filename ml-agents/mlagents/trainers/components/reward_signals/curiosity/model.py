...
class CuriosityModel(object):
    def __init__(
        ...
        self.pca = pk.load(open("/Users/alehuger/Imperial/rl/research_project/code/AnimalAI-Olympics/ml-agents/mlagents/"
                                "trainers/components/reward_signals/curiosity/pca.pkl", 'rb'))

        encoded_state, encoded_next_state = self.create_curiosity_pca_encoders()
    ...
    def pca_transform(self, visual_input_tensor) -> Tuple[tf.Tensor]:

        visual_obs_flatten = tf.reshape(visual_input_tensor, (-1, 84*84*3))
        visual_obs_flatten_mean = visual_obs_flatten - self.pca.mean_
        res = tf.linalg.matmul(visual_obs_flatten_mean, self.pca.components_.T)

        return res

    def create_curiosity_pca_encoders(self) -> Tuple[tf.Tensor, tf.Tensor]:
        """
        Creates state encoders for current and future observations.
        Used for implementation of ﻿Curiosity-driven Exploration by Self-supervised Prediction
        See https://arxiv.org/abs/1705.05363 for more details.
        ...
        if self.policy.vis_obs_size > 0:
            ...
            for i in range(self.policy.vis_obs_size):
                ...
                # Create the encoder ops for current and next visual input.
                # Note that these encoders are siamese.
                encoded_visual = self.pca_transform(self.policy.visual_in[i])
                encoded_next_visual = self.pca_transform(self.policy.visual_in[i])

                visual_encoders.append(encoded_visual)
                next_visual_encoders.append(encoded_next_visual)

            hidden_visual = tf.concat(visual_encoders, axis=1)
            print(hidden_visual.shape)
            hidden_next_visual = tf.concat(next_visual_encoders, axis=1)
            encoded_state_list.append(hidden_visual)
            encoded_next_state_list.append(hidden_next_visual)
        ...
        if self.policy.vec_obs_size > 0:
            ...
            encoded_state_list.append(encoded_vector_obs)
            print(encoded_vector_obs.shape)
            encoded_next_state_list.append(encoded_next_vector_obs)
